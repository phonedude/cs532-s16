...
def printclust2file(clust, out,labels=None, n=0):
        ...
        if clust.id < 0:
            ...
            if labels is None:
                ...
                out.write("%s\n"%labels[clust.id])
        if clust.left is not None: printclust2file(clust.left, out,labels=labels, n=n + 1)
        if clust.right is not None: printclust2file(clust.right, out,labels=labels, n=n + 1)
...
def kcluster_toFile(vecs, distance=pearson, k=4,out=None):
    ranges = [(min([vec[i] for vec in vecs]), max([vec[i] for vec in vecs])) for i in range(len(vecs[0]))]
    clusters = [[random.random() * (ranges[i][1] - ranges[i][0]) + ranges[i][0] for i in range(len(vecs[0]))]for j in range(k)]
...
def scaledown(data, distance=pearson, rate=0.01):
    ...
    for m in range(0, 1000):
        ...
        for k in range(n):
            for j in range(n):
                if j == k: continue
                # print("realdist = %f ,fakedist= %f "%(realdist[j][k],fakedist[j][k]))
